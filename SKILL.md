...
- 2026-09（排版溢出根治 F11）：全面重构模板宏体系以消除 framed/multicols 冲突、标题缺\par 穿透、表格绝对定宽超标、长单词溢出、填空宏不可断行与答题空白过度膨胀等六大根源问题。核心措施包括：taskbox 开放流架构（\colorbox 浅色横幅 + 自由展开）、languagecard 独立横幅+强制换段、生词表列宽压缩至总宽≤155mm、填空宏采用 ulem 宏包\uline 自适应折行、答题空间注入弹性胶 plus/minus、全局\emergencystretch 防御微溢出。编写须严格遵循容器开放律、双版窄边对齐律、英文长词防御律、填空可折行律、空间配比律五大铁律。详见 references/typesetting-specifications.md 与 docs/reports/typesetting-overflow-analysis.md。
